    ...
    <Link href={`/auto/${v.id}`} className="group">
      <div className="bg-white rounded-[2rem] overflow-hidden shadow-lg border border-gray-100 hover:shadow-2xl transition-all transform hover:-translate-y-2">
        <div className="bg-gray-200 h-56 flex items-center justify-center relative">
          ...
          <div className={`absolute top-4 left-4 ${color === 'black' ? 'bg-black text-white' : 'bg-yellow-500 text-black'} text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest`}>
            {v.tipo_publicacion === 'Propio' ? 'Stock Maxi' : 'Oportunidad'}
          </div>
        </div>
        <div className="p-8">
          <h3 className="text-xl font-black text-gray-900 mb-1 uppercase leading-none">{v.titulo}</h3>
